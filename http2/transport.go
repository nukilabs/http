...
import (
	...
	"github.com/nukilabs/http/internal/httpcommon"
...
// Priority represents the priority param of a stream.
...
type Transport struct {
	// MaxUploadBufferPerConnection is the maximum size of the flow control window for
	// data received on a connection. A valid value is at least 64KiB and
	// less than 4MiB. If invalid, a default value is used.
...
var errRequestCanceled = errors.New("nukilabs/http: request canceled")
...
var errExtendedConnectNotSupported = errors.New("nukilabs/http: extended connect not supported by peer")
...
func encodeRequestHeaders(req *http.Request, addGzipHeader bool, peerMaxHeaderListSize uint64, pseudoHeaderOrder []string, headerf func(name, value string)) (httpcommon.EncodeHeadersResult, error) {
...
func (rl *clientConnReadLoop) cleanup() {
	...
	idleTime := time.Since(cc.lastActive)
...
func (b transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("nukilabs/http: server replied with more than declared Content-Length; truncated")
				cs.abortStream(err)
			}
			cs.readErr = err
