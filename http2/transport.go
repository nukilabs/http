...
var errRequestCanceled = errors.New("github.com/nukilabs/http: request canceled")
...
var errExtendedConnectNotSupported = errors.New("github.com/nukilabs/http: extended connect not supported by peer")
...
func validateHeaders(hdrs http.Header) string {
	for k, vv := range hdrs {
		if !httpguts.ValidHeaderFieldName(k) && k != ":protocol" && k != http.HeaderOrderKey {
...
func (b transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("github.com/nukilabs/http: server replied with more than declared Content-Length; truncated")
				cs.abortStream(err)
			}
			cs.readErr = err
