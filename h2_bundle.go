...
//go:generate bundle -o=h2_bundle.go -prefix=http2 github.com/nukilabs/http/http2
...
var http2errExtendedConnectNotSupported = errors.New("github.com/nukilabs/http: extended connect not supported by peer")
...
func http2validateHeaders(hdrs Header) string {
	for k, vv := range hdrs {
		if !httpguts.ValidHeaderFieldName(k) && k != ":protocol" && k != HeaderOrderKey {
